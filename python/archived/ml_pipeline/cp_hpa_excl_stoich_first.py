...
CONFIG = {
    ...
    "BASE_OUTPUT_DIR": Path("./randomforest/hpa_one_excl_stoich_features"),
...
def plot_dimensionality_control(all_results: List[Dict], stats: Dict, output_dir: Path):
    ...
    # ================================================================
    arms = {
        'Pairwise (1)':            {'vals': pair_vals,     'colour': '#b0b0b0'},
        f'Pair + noise (1+{n_pad})': {'vals': padded_vals,   'colour': '#f4a582'},
        'Hyper-exclusive (5)':     {'vals': excl_vals,     'colour': '#9e7bb5'},
        'Hyper no-stoich (6)':     {'vals': nostoich_vals, 'colour': '#92c5de'},
        'Hypergraph full (10)':    {'vals': hyper_vals,    'colour': '#4393c3'},
    }
    n_arms = len(arms)
    bar_width = 0.15
    ...
    ax1.set_xlabel('Pairwise PR-AUC (1 feature)')
    ...
    ax2.plot([lo, hi], [lo, hi], 'r--', linewidth=2, label='y = x')
    ax2.set_xlabel('Pairwise PR-AUC (1 features)')
