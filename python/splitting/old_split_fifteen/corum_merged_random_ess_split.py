...
The analysis population is proteins that are BOTH:
  - labelled (Essential or Non-essential in lu_essentiality_protein.csv)
  - structured (present in corum_mapping_struct.csv, i.e. have a group_id)

All rates, deviations, and the essentiality-by-size table use this
population as the denominator. Proteins that are Unknown or have no
structural group are carried through the split CSV for completeness but
excluded from all rate calculations.

OUTPUTS
-------
  corum_ess_merged_splits.csv              : one row per (protein, split_index)
  corum_ess_merged_groups.csv             : per-split label balance statistics
  corum_ess_merged_split_summary.txt      : human-readable summary + profile
  corum_avg_essentiality_by_group_size.csv: essentiality rate per group size
...
CONFIG = {
    ...
    # Output files
    'output_all_splits':  '/Users/anitaapplegarth/github/dphil/protein_complexes/data/lookup_tables/corum/ess_protein_merged_splits.csv',
    'output_balance':     '/Users/anitaapplegarth/github/dphil/protein_complexes/data/lookup_tables/corum/ess_merged_groups.csv',
    'output_summary':     '/Users/anitaapplegarth/github/dphil/protein_complexes/data/lookup_tables/corum/ess_merged_split_summary.txt',
    'output_ess_by_size': '/Users/anitaapplegarth/github/dphil/protein_complexes/data/lookup_tables/corum/avg_essentiality_by_group_size.csv',
...
def attempt_split(buckets, group_protein_map, no_group_proteins,
    ...
    for lbl in ('Essential', 'Non-essential', 'Unknown'):
...
def main():
    ...
    # protein_to_label: Essential / Non-essential only (Unknown -> absent)
    protein_to_label = {}
    ...
    for i in range(CONFIG['n_splits']):
        ...
        while attempts < CONFIG['max_attempts']:
            ...
            if sz_dev <= CONFIG['max_size_deviation'] and \
               dev    <= CONFIG['max_ess_rate_deviation']:
                accepted = True
                break
        ...
        for bname in ('large', 'medium', 'small', 'singleton'):
            ...
            # Ess rate for bucket: analysis population only
            ...
            bucket_breakdown[bname] = {
                ...
                'test_ess_pct': round(100 * bc['Essential'] / lab, 1) if lab > 0
                                else float('nan'),
            }
        ...
        bal_row = {
            ...
            **{f'{b}_test_pct':     round(bucket_breakdown[b]['test_pct'], 1)
               for b in ('large', 'medium', 'small', 'singleton')},
            **{f'{b}_test_ess_pct': bucket_breakdown[b]['test_ess_pct']
               for b in ('large', 'medium', 'small', 'singleton')},
        }
    ...
    lines.append(f"Size tolerance:                   ±{CONFIG['max_size_deviation']}pp  "
                 ...
                 f"{100*CONFIG['test_ratio']+CONFIG['max_size_deviation']:.0f}%)")
    lines.append(f"Balance tolerance:                ±{CONFIG['max_ess_rate_deviation']}pp")
    ...
    for r in balance_records:
        lines.append(
            f"{r['split_index']:>6}  "
            f"{r['large_test_n']:>8} {r['large_test_pct']:>4.1f}% {r['large_test_ess_pct']:>5.1f}%  "
            f"{r['medium_test_n']:>9} {r['medium_test_pct']:>4.1f}% {r['medium_test_ess_pct']:>5.1f}%  "
            f"{r['small_test_n']:>8} {r['small_test_pct']:>4.1f}% {r['small_test_ess_pct']:>5.1f}%  "
