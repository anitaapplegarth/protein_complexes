...
OUTPUT_DIR    = PROJECT_ROOT / "python/analysis/stoichiometry/compare_regression"
...
# Matched protein/base features — the equivalent using unique protein counts only
PROTEIN_FEATURES = [
...
def main():
    ...
    # ------------------------------------------------------------------
    # 5. Protein features: univariate logistic regressions
    # ------------------------------------------------------------------
    print(f"\n{'=' * 70}")
    print("PROTEIN FEATURES — UNIVARIATE LOGISTIC REGRESSIONS (standardised)")
    print("=" * 70)

    # Check protein feature columns exist
    prot_missing = [c for c in PROTEIN_FEATURES if c not in df.columns]
    if prot_missing:
        print(f"WARNING: protein feature columns not found: {prot_missing}")
        print("Skipping protein feature analysis.")
    else:
        X_prot_raw = df[PROTEIN_FEATURES].astype(float)
        X_prot_z = (X_prot_raw - X_prot_raw.mean()) / X_prot_raw.std()

        for feat in PROTEIN_FEATURES:
            Xi = sm.add_constant(X_prot_z[feat])
            model = sm.Logit(y, Xi).fit(disp=False)
            res = summarise_logit(model, ["const", feat])
            res["Model"] = f"Univariate: {feat}"
        ...
        # ------------------------------------------------------------------
        # 6. Protein features: multivariate logistic regression
        # ------------------------------------------------------------------
        print(f"\n{'=' * 70}")
        print("PROTEIN FEATURES — MULTIVARIATE LOGISTIC REGRESSION (standardised)")
        print("=" * 70)

        X_prot_z_const = sm.add_constant(X_prot_z)
        model_prot = sm.Logit(y, X_prot_z_const).fit(disp=False)
        res_prot = summarise_logit(
            model_prot, ["const"] + PROTEIN_FEATURES
        )
        res_prot["Model"] = "Multivariate: protein only"
        all_results.append(res_prot)

        print(model_prot.summary2())

        # ------------------------------------------------------------------
        # 7. Nested LR test: does stoichiometry add signal over protein features?
        # ------------------------------------------------------------------
        print(f"\n{'=' * 70}")
        print("NESTED LIKELIHOOD RATIO TEST")
        print("  Reduced model: target ~ protein features only")
        print("  Full model:    target ~ protein features + stoich features")
        print("=" * 70)

        # Full model: protein + stoich features combined
        all_feat_cols = PROTEIN_FEATURES + features
        X_all_raw = df[all_feat_cols].astype(float)
        X_all_z = (X_all_raw - X_all_raw.mean()) / X_all_raw.std()
        X_all_z_const = sm.add_constant(X_all_z)

        model_full = sm.Logit(y, X_all_z_const).fit(disp=False)
        res_full = summarise_logit(
            model_full, ["const"] + all_feat_cols
        )
        res_full["Model"] = "Multivariate: protein + stoich"
        all_results.append(res_full)

        print(f"\n  Reduced model (protein only):")
        print(f"    Log-likelihood: {model_prot.llf:.2f}")
        print(f"    Pseudo R²:      {model_prot.prsquared:.4f}")
        print(f"    AIC:            {model_prot.aic:.2f}")

        print(f"\n  Full model (protein + stoich):")
        print(f"    Log-likelihood: {model_full.llf:.2f}")
        print(f"    Pseudo R²:      {model_full.prsquared:.4f}")
        print(f"    AIC:            {model_full.aic:.2f}")

        # LR test
        nested_lr_stat = 2 * (model_full.llf - model_prot.llf)
        nested_lr_df = len(features)  # number of extra stoich features
        nested_lr_pval = chi2.sf(nested_lr_stat, nested_lr_df)

        print(f"\n  LR statistic:       {nested_lr_stat:.2f}")
        print(f"  Degrees of freedom: {nested_lr_df} "
              f"({len(features)} stoich features added)")
        print(f"  p-value:            {nested_lr_pval:.2e}")

        if nested_lr_pval < 0.05:
            print(f"  → Stoichiometry features add significant signal "
                  f"beyond protein features alone")
        else:
            print(f"  → Stoichiometry features do NOT add significant "
                  f"signal beyond protein features")

        # Also test the reverse: do protein features add to stoich?
        print(f"\n{'=' * 70}")
        print("REVERSE NESTED LR TEST (for completeness)")
        print("  Reduced model: target ~ stoich features only")
        print("  Full model:    target ~ stoich features + protein features")
        print("=" * 70)

        rev_lr_stat = 2 * (model_full.llf - model_multi.llf)
        rev_lr_df = len(PROTEIN_FEATURES)
        rev_lr_pval = chi2.sf(rev_lr_stat, rev_lr_df)

        print(f"\n  Reduced (stoich only) LL:  {model_multi.llf:.2f}")
        print(f"  Full (stoich + protein) LL: {model_full.llf:.2f}")
        print(f"  LR statistic:       {rev_lr_stat:.2f}")
        print(f"  Degrees of freedom: {rev_lr_df}")
        print(f"  p-value:            {rev_lr_pval:.2e}")

        if rev_lr_pval < 0.05:
            print(f"  → Protein features add significant signal "
                  f"beyond stoichiometry features alone")
        else:
            print(f"  → Protein features do NOT add significant "
                  f"signal beyond stoichiometry features")
        ...
        comparison = pd.DataFrame({
            "Model": [
                "Intercept only",
                "Stoich features only",
                "Protein features only",
                "Protein + Stoich features",
            ],
            "Log-Likelihood": [
                null_model.llf,
                model_multi.llf,
            ...
            "Pseudo R²": [
                0.0,
                model_multi.prsquared,
                model_prot.prsquared,
        ...
        comp_path = outdir / "model_comparison_summary.csv"
        comparison.to_csv(comp_path, index=False)
        print(f"\nModel comparison saved: {comp_path}")

        # Forest plot for full model
        forest_full_path = outdir / "nested_full_model_forest.png"
        plot_forest(
            res_full,
            "Protein + Stoichiometry → Drug Target\n"
            "(Full Model, Nested LR Test)",
            forest_full_path,
        )
