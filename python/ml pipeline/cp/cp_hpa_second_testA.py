...
CONFIG = {
    ...
    "BASE_OUTPUT_DIR": Path("./randomforest/cp_hpa_second_testA"),
...
def print_statistical_summary(stats: Dict):
    ...
    print(f"  {'Annotation presence   (flag − hypergraph)':<44} "
          f"{ann['mean_diff']:>+9.4f} {ann['wins']:>3}/{ann['losses']:<3} {ann['p_greater']:>12.4f}")
    print(f"  {'Stoichiometry values  (hb-graph − flag)':<44} "
          f"{val['mean_diff']:>+9.4f} {val['wins']:>3}/{val['losses']:<3} {val['p_greater']:>12.4f}")
    print(f"  {'-'*76}")
    print(f"  {'TOTAL  (hb-graph − hypergraph)':<44} "
          f"{tot['mean_diff']:>+9.4f} {tot['wins']:>3}/{tot['losses']:<3} {tot['p_greater']:>12.4f}")
