...
CORUM variant of cp_noise_injection.py. Identical protocol, feature lists,
grids, seeds and figure; only the input directory and the output directory
name differ, so the two runs are directly comparable.
"""
...
CONFIG = {
    ...
    # Tags the output directory so a CORUM run never overwrites the cp one.
    "DATABASE": "corum",
...
def main():
    ...
    print(f"  Data   : {CONFIG['DATABASE'].upper()}  ({CONFIG['DATA_DIR']})")
