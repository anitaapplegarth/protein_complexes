...
# SPLITS_FILE   — filename in the database directory containing the split assignments
...
CONFIG = {
    ...
    # --- Database ---
    # Set to "cp" or "corum" — DATA_DIR is derived automatically
    "DATABASE": "corum",

    # --- Paths ---
    "DATA_ROOT":       Path("/Users/anitaapplegarth/github/dphil/protein_complexes/data/lookup_tables/"),
...
def get_data_dir() -> Path:
    return CONFIG["DATA_ROOT"] / CONFIG["DATABASE"]
...
def get_output_dir() -> Path:
    ...
    d = CONFIG["BASE_OUTPUT_DIR"] / f"{model}_{db}_{task}_noise_injection"
...
def load_all_features() -> pd.DataFrame:
    ...
    hg_df   = pd.read_csv(get_data_dir() / CONFIG["PROTEIN_FEATURES_FILE"])
    pair_df = pd.read_csv(get_data_dir() / CONFIG["PAIRWISE_FEATURES_FILE"])
...
def load_splits() -> pd.DataFrame:
    ...
    splits_df = pd.read_csv(get_data_dir() / meta["SPLITS_FILE"])
...
def write_summary(
    ...
    lines = [
        ...
        sep,
        f"  Database      : {CONFIG['DATABASE'].upper()}",
...
# =======================================================
# CUMULATIVE TABLE-S4 CSV  (append-safe across runs)
# =======================================================

def save_supplementary_csv(
    hyper_ranked: pd.DataFrame,
    pair_ranked: pd.DataFrame,
    prauc_f1_df: pd.DataFrame,
    output_dir: Path,
    ...
    """
    Save two cumulative CSVs that can be read straight into the
    supplementary tables without manual transcription.

    1. table_s4_cumulative.csv  — one row per feature per run,
       with database/task/model labels.  Append-safe: re-running
       the same (database, task, model) combination overwrites
       only those rows.

    2. table_s1s2_cumulative.csv — one row per (split × model-arm)
       with PR-AUC and the three model arms (rand/pair/hyper).
       Enough to compute mean±SD and win/loss/% improvement.

    Both files live in BASE_OUTPUT_DIR (one level above the
    per-run output directory) so they accumulate across runs.
    """
    db    = CONFIG["DATABASE"]
    task  = CONFIG["TASK"]
    model = CONFIG["MODEL_TYPE"]
    cumul_dir = CONFIG["BASE_OUTPUT_DIR"]
    cumul_dir.mkdir(parents=True, exist_ok=True)

    # ── Table S4: feature-level hit rates ─────────────────────
    s4_path = cumul_dir / "table_s4_cumulative.csv"
    rows = []
    for label, ranked in [("hyper", hyper_ranked), ("pair", pair_ranked)]:
        for _, r in ranked.iterrows():
            ...
            if "hit_rate" in r:
                ...
                row["binom_p"]  = r["binom_p"]
            rows.append(row)

    new_df = pd.DataFrame(rows)
    if s4_path.exists():
        old = pd.read_csv(s4_path)
        # Drop previous rows for this exact run
        mask = (
            (old["database"] == db)
            & (old["task"] == task)
            & (old["model"] == model)
        ...
        combined = new_df
    combined.to_csv(s4_path, index=False)
    print(f"   Saved: {s4_path}  ({len(new_df)} rows appended)")

    # ── Table S1/S2: per-split PR-AUC for win/loss computation ─
    s1_path = cumul_dir / "table_s1s2_cumulative.csv"
    perf = prauc_f1_df[prauc_f1_df["split_index"] != "MEAN_OVER_SPLITS"].copy()
    perf.insert(0, "database", db)
    perf.insert(1, "task", task)
    perf.insert(2, "model", model)

    if s1_path.exists():
        old = pd.read_csv(s1_path)
        mask = (
            (old["database"] == db)
            & (old["task"] == task)
            & (old["model"] == model)
        ...
        old = old[~mask]
        combined = pd.concat([old, perf], ignore_index=True)
    else:
        combined = perf
    combined.to_csv(s1_path, index=False)
    print(f"   Saved: {s1_path}  ({len(perf)} rows appended)")
...
def run_summary_only(output_dir: Path):
    ...
    # Cumulative supplementary-table CSV
    if not prauc_f1_df.empty:
        save_supplementary_csv(hyper_ranked, pair_ranked, prauc_f1_df, output_dir)
...
if __name__ == '__main__':
    ...
    print(f"  Database: {CONFIG['DATABASE'].upper()}")
    ...
    if SUMMARY_ONLY:
        ...
        # Cumulative supplementary-table CSV (append-safe across runs)
        print("\n8. Updating cumulative supplementary CSVs...")
        save_supplementary_csv(hyper_ranked, pair_ranked, prauc_f1_df, output_dir)
