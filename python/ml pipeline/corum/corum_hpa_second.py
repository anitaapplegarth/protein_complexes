...
CONFIG = {
    ...
    "BASE_OUTPUT_DIR": Path("./randomforest/corum_hpa_second_testA"),
    ...
    "RAW_STOICH_FILE": Path("/Users/anitaapplegarth/github/dphil/protein_complexes/data/lookup_tables/corum/stoich_protein.csv"),
    ...
    #     "SPLITS_FILE":     "hpa_protein_splits_unstrat.csv",
    #     "BASE_OUTPUT_DIR": Path("./randomforest/corum_hpa_second_testA_unstrat"),
...
def derive_annotation_flag(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Derived from stoich_MedianRatio. In corum_hypergraph_features.ipynb the ratio is
...
if __name__ == "__main__":
    ...
    print(f"  Task   : Drug Target Prediction (HPA)")
    ...
    for i, split_idx in enumerate(split_indices, start=1):
        ...
        try:
            ...
            print(f"train={result['n_train']} ({result['train_ess_pct']:.1f}% pos)  "
                  f"test={result['n_test']} ({result['test_ess_pct']:.1f}% pos)  |  "
    ...
    with open(output_dir / 'statistical_summary.txt', 'w') as f:
            ...
            f.write("Task: Drug Target Prediction (HPA)\n")
            f.write("\nRUN PROVENANCE\n")
            f.write(f"{'-'*70}\n")
            f.write(f"Run at              : "
